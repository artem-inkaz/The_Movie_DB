...
class MovieActorRepositoryImpl: MovieActorRepository {
    ...
    private val moviesDB = MoviesDataBase.instance.getMovieActorDao()
    private val movieActorMapper = MovieAndActorMapper()

    override fun getAll(): Single<List<MovieActor>> {
        return moviesDB.getAll().map { movieActorMapper.fromLocalDataBase(it) }
    }
    ...
    override fun add(moveAndActor: MovieActor): Completable {
        return moviesDB.add(movieActorMapper.toLocalDataBase(moveAndActor))
    }

    override fun addAll(moveAndActors: List<MovieActor>) {
        return moviesDB.addAll(movieActorMapper.toLocalDataBase(moveAndActors))
    }
